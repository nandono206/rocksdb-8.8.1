...
DEFINE_string(text_path, "", "The text file path.");
...
Status TraceAnalyzer::PutCF(uint32_t column_family_id, const Slice& key,
                            const Slice& value) {
//  fprintf(stdout, "IN PUT CF\n");
//  fprintf(stdout, "value:%s\n", ROCKSDB_NAMESPACE::LDBCommand::StringToHex(value.ToString()).c_str());
...
Status TraceAnalyzer::PutEntityCF(uint32_t column_family_id, const Slice& key,
                                  const Slice& value) {
//  fprintf(stdout, "IN PUT ENTITY CF\n");
